...
def main():
    ...
    class MistralForSequenceEmbedding(torch.nn.Module):
        ...
        def forward(
            ...
            selected_layers = [hidden_states[layer][:, -1, :].unsqueeze(1) for layer in range(1, layers+1)]
            last_token_hidden_states = torch.cat(selected_layers, dim=1)
            
            embeddings = self.attn_pool(last_token_hidden_states)
